...
//        bookList.stream().filter((Book book) -> book.getCategory().equals("여행")).forEach(filterdBook -> System.out.println("책 제목 : " + filterdBook.getBookName()));

//        System.out.println("[ 가격이 16200원 이하인 책 제목 조회 ]");
//        bookList.stream().filter((Book book) -> book.getPrice()<=16200).forEach(filterdBook -> System.out.println("책 제목 : " + filterdBook.getBookName()));

//        System.out.println("[ 책 제목에 \"경제\" 라는 용어가 들어간 책 제목 조회하기 ]");
//        bookList.stream().filter((Book book) -> book.getBookName().contains("경제")).forEach(filterdBook -> System.out.println("책 제목 : " + filterdBook.getBookName()));

//        System.out.println("[ 가격이 가장 비싼 책 가격 조회하기 ]");
//        Double max = bookList.stream().mapToDouble(Book::getPrice).max().getAsDouble();
//        System.out.print("제일 비싼 책 가격 : " + max +"원");

//        System.out.println("[ 카테고리가 IT인 책들의 가격 합 조회하기 ]");
//        Double sum = bookList.stream().filter((Book book) -> book.getCategory().equals("IT")).mapToDouble(Book::getPrice).sum();
//        System.out.print("카테고리가 IT인 책들의 가격 합 : " + sum +"원");
        ...
        bookList.stream()
                ...
                .forEach(filteredBook -> {
                    System.out.println("책 제목 : " + filteredBook.getBookName());
                    System.out.println("책 할인 가격 : " +filteredBook.getPrice()+"원\n");
                });
    }
}
